...
export default function Home() {
    ...
    const contractAddress = "0x833853b7B983367c7874406F5d25bBAe783ef190"; // Replace with your smart contract address
    ...
    useEffect(() => {
        async function fetchNFTs() {
			...
			// Get the balance of NFTs owned by the signer
            const signerAddress = await signer.getAddress();
            const balance = await contract.balanceOf(signer.getAddress());
            console.log("Balance of NFTs:", balance.toString());

			// Get the list of token IDs owned by the signer
            const tokenIds = [];
            for (let i = 0; i < balance.toNumber(); i++) {
                const tokenId = await contract.tokenOfOwnerByIndex(signerAddress, i);
                tokenIds.push(tokenId);
            }
            console.log("Token IDs owned by the signer:", tokenIds);

            // Fetch metadata for each token ID and add it to the state
